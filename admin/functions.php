...
function displaySetForm($set = null) {
  ...
  $isDefunct = ($metaData->getVar("row_flag") == CDM_RSTAT_DEF);
...
function submitSetForm() {
	...
	*/
	...
	//save new values to object
  	...
  	if (($old_rstat != CDM_RSTAT_DEF) AND ($row_flag == CDM_RSTAT_DEF)) { //properly defunct the set and its child codes
  		$metaData->setDefunct();
  	} else {
...
function adminSelectCode($setName = null,$setLang = CDM_DEF_LANG) {
	if ($setName==null) { //ask user to select a codeset
		adminSelectSet(true);
		...
		$langName = CDMGetCode('LANGUAGE',$setLang,$setLang);
...
function displayCodeForm($cd_set,$cd,$cd_lang=CDM_DEF_LANG) {
  ...
  //$xoopsTpl->assign('lang_pagetitle',_AM_SACC_PAGETITLE4);
    ...
    Global $xoopsUser;
    if (empty($xoopsUser)) redirect_header(CDM_URL."/index.php?codeSet=".CDM_DEF_SET,1,_MD_CDM_ERR_5);
    ...
    if ($codeData) {
      ...
      $cd_len = intval($meta->getVar('cd_len'));
      $val_len = intval($meta->getVar('val_len'));      
...
function submitCodeForm() {
  global $HTTP_POST_VARS;
  extract($HTTP_POST_VARS);  //retrieve posted data values 
...
function adminSelectBulkUpload() {
	...
	*/
	global $xoopsDB, $xoopsOption, $xoopsTpl, $HTTP_POST_VARS, $HTTP_GET_VARS, $xoopsUser;

    //can't use templates in 2.0.13 adminside
	//$xoopsOption['template_main'] = 'cdm_db_update.tpl';	// Set the template page to be used

	//require_once XOOPS_ROOT_PATH."/header.php";	// include the main header file
    //Check to see if user logged in
    if (empty($xoopsUser)) redirect_header(CDM_URL."/admin/adminupload.php",1,_MD_CDM_ERR_5);
...
*
* @version 1
* @param string $fileLoc File location on user local machine
*/
function adminBulkUpload($fileloc) {
	 ...
	 * @global array Form GET variable array
	*/	
	global $xoopsDB, $xoopsOption, $xoopsTpl, $HTTP_POST_VARS, $HTTP_GET_VARS;
	extract($HTTP_POST_VARS);
