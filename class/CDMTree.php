...
class CDMObjectTree {
	 ...
	 **/
	function CDMObjectTree(&$objectArr, $myId, $parentId)
	{
	 ...
	 **/
	function &makeSelBox($name, $fieldName, $selected='', $prefix='-', $addEmptyOption = false, $key=0)
    {
	...
	* @param   string  $prefix_curr  String to indent the current item
    */
    function _getSelArrs($dispFld, $key, &$ret, $prefix_orig, $prefix_curr = '') {
       if (!empty($key) || ($key > 0)) {
    ...
    *
    * @return an associative array 'code => value' of the code set in tree hierarchy order
    */
    function getSelArr($dispFld = 'cd_value',$prefix = '-') {
    	$ret = array();
    	foreach (array_keys($this->_tree) as $key) {
        	if (!isset($this->_tree[$key]['parent'])) {//don't duplicate items that are children of others
